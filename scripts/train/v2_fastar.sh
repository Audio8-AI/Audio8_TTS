...
# Stage-2: train the frozen fast AR branch on top of the stage-1 slow-AR model.
# - model weights: latest stage-1 export (auto-detected below)
# - freeze_slow_ar=true, freeze_fast_ar=false, slow_ar_only=false
# - base_loss_weight=0.0 -> loss is purely the fast-AR codebook cross-entropy
# - data/prompt format stays identical to stage-1 (pair ref text + ref codes)

source "$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)/common.sh"
cd "$PROJECT_ROOT"

PID_FILE="${PID_FILE:-train_formal_3node_audio8_falcon_fastar_v2.pid}"
printf '%s\n' "$$" > "$PID_FILE"
...
TRAIN_JSONL="${TRAIN_JSONL:?Set TRAIN_JSONL to the v2 manifest}"
OUTPUT_DIR="${OUTPUT_DIR:-$OUTPUT_ROOT/v2_fastar}"
EXPORT_DIR="${EXPORT_DIR:-$EXPORT_ROOT/v2_fastar}"
...
LOCAL_NPY_CACHE_DIR="${LOCAL_NPY_CACHE_DIR:-$CACHE_ROOT/v2_fastar}"
...
if [[ -z "$MODEL_PATH" ]]; then
  if [[ -f "$STAGE1_EXPORT_DIR/model.safetensors" ]]; then
    MODEL_PATH="$STAGE1_EXPORT_DIR"
  elif [[ -f "$STAGE1_EXPORT_DIR/checkpoint-26000/model.safetensors" ]]; then
    MODEL_PATH="$STAGE1_EXPORT_DIR/checkpoint-26000"
  else
    echo "No stage-1 export found under $STAGE1_EXPORT_DIR" >&2
    exit 1
  fi
fi
echo "Stage-2 starting from $MODEL_PATH"
