"""Neural audio codec bundled with the audio8_tts checkpoint."""
...
class ArkttsDownsampleQuantizer(nn.Module):
    def __init__(self):
        ...
        transformer_config = ArkttsCodecTransformerConfig(
            n_layer=8,
            n_head=16,
            dim=1024,
        ...
        self.pre_module = ArkttsCodecWindowTransformer(
            transformer_config, 1024, window_size=128
        )
...
class ArkttsCodec(nn.Module):
    """Encode 44.1 kHz mono audio to ten codebooks and decode it back."""
    ...
    def encode(self, audio, audio_lengths=None):
        """Return padded indices ``[B, 10, T]`` and valid frame lengths."""
        if audio.ndim == 2:
            audio = audio[:, None]
    ...
    def decode(self, codes):
        """Decode a dense ``[B, 10, T]`` integer code tensor."""
